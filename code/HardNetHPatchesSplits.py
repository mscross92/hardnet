    ...
    for batch_idx, data in pbar:
        if load_triplets:
            ...
            if batch_idx == 0:
                ...
                plt.subplot(131)
                plt.imshow((data_a[index,0,:,:].numpy()*255).astype('uint8'), cmap='gray',vmax=255,vmin=0) 
                plt.gca().set_xticks([])
                plt.gca().set_yticks([])
                plt.title('Anchor', fontsize=12)
                plt.subplot(132)
                plt.imshow((data_p[index,0,:,:].numpy()*255).astype('uint8'), cmap='gray',vmax=255,vmin=0) 
                plt.title('Positive', fontsize=12)
                plt.gca().set_xticks([])
                plt.gca().set_yticks([])
                plt.subplot(133)
                plt.imshow((data_n[index,0,:,:].numpy()*255).astype('uint8'), cmap='gray',vmax=255,vmin=0) 
