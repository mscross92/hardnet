...
cd "$RUNPATH"
python -utt ./code/HardNetTurbidEvaluate.py --hpatches-split="$DATASETS/hpatches/"  --gpu-id=4 --fliprot=False --model-dir=model_HPatches_b --dataroot "$DATASETS/turbid/" --lr=10 --w1bsroot "$DATASETS/wxbs-descriptors-benchmark/code/" --batch_hard 0 --batch-reduce="random_global" --imageSize 32 --epochs=10 --margin=1.0 --log-dir logs  --experiment-name=hpatches_a_aug_lr10/ | tee -a "$DATALOGS/log_HardNet_orthlr10_aug_HPatches_a.log"
# python -utt ./code/HardNetHPatchesSplits.py --hpatches-split=f"$DATASETS/hpatches_splits/hpatches_split_c_train.pt"  --gpu-id=4 --fliprot=True --model-dir=model_HPatches_c --dataroot "$DATASETS/PhotoTourism/" --lr=10.0 --w1bsroot "$DATASETS/wxbs-descriptors-benchmark/code/" --n-triplets=15000000 --imageSize 32 --log-dir logs  --experiment-name=hpatches_c_aug_lr10/ | tee -a "$DATALOGS/log_HardNet_orthlr10_aug_HPatches_c.log"
