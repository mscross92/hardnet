...
cd "$RUNPATH"
CUDA_LAUNCH_BLOCKING=1 python -utt ./code/HardNetHPatchesSplits.py --hpatches-split="$DATASETS/hpatches/"  --gpu-id=4 --fliprot=True --model-dir=model_HPatches_a --dataroot "$DATASETS/turbid/" --lr=10.0 --w1bsroot "$DATASETS/wxbs-descriptors-benchmark/code/" --batch_hard 1 --n-triplets=11697 --imageSize 29 --log-dir logs  --experiment-name=hpatches_a_aug_lr10/ | tee -a "$DATALOGS/log_HardNet_orthlr10_aug_HPatches_a.log"
