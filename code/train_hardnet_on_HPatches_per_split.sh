...
cd "$RUNPATH"
python -utt ./code/HardNetHPatchesSplits.py --hpatches-split="$DATASETS/hpatches/hpatches_split_a_train.pt"  --gpu-id=4 --fliprot=True --model-dir=model_HPatches_a --dataroot "$DATASETS/turbid/" --lr=10.0 --w1bsroot "$DATASETS/wxbs-descriptors-benchmark/code/" --n-triplets=409395 --imageSize 29 --log-dir logs  --gpu_id=0 --experiment-name=hpatches_a_aug_lr10/ | tee -a "$DATALOGS/log_HardNet_orthlr10_aug_HPatches_a.log"
