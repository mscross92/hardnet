...
cd "$RUNPATH"
python -utt ./code/HardNetHPatchesSplits.py --hpatches-split="$DATASETS/hpatches/"  --gpu-id=4 --fliprot=False --model-dir=model_HPatches_a --dataroot "$DATASETS/turbid/" --lr=10 --fliprot=True --margin=1.0 --w1bsroot "$DATASETS/wxbs-descriptors-benchmark/code/" --batch_hard 1 --batch-reduce="random_global" --imageSize 32 --epochs=10 --log-dir logs  --experiment-name=hpatches_a_aug_lr10/ | tee -a "$DATALOGS/log_HardNet_orthlr10_aug_HPatches_a.log"
