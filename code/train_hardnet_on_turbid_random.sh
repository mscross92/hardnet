...
cd "$RUNPATH"
python -utt ./code/HardNetHPatchesSplits.py --hpatches-split="$DATASETS/hpatches/"  --gpu-id=4 --fliprot=True --model-dir=model_HPatches_a --dataroot "$DATASETS/turbid/" --lr=0.01 --w1bsroot "$DATASETS/wxbs-descriptors-benchmark/code/" --batch_hard 0 --batch-reduce="random_global" --n-triplets=58485 --imageSize 29 --epochs=50 --log-dir logs  --experiment-name=hpatches_a_aug_lr10/ | tee -a "$DATALOGS/log_HardNet_orthlr10_aug_HPatches_a.log"
